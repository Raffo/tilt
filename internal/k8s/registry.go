...
const labelRegistryHostnameFromLocal = "tilt.dev/registry-hostname-from-local"
const labelRegistryPortFromLocal = "tilt.dev/registry-port-from-local"
const labelRegistryHostnameFromCluster = "tilt.dev/registry-hostname-from-cluster"
const labelRegistryPortFromCluster = "tilt.dev/registry-port-from-cluster"
...
func (r *registryAsync) inferRegistryFromTiltNodeLabels(ctx context.Context) container.Registry {
	...
	registryHostnameFromLocal := labels[labelRegistryHostnameFromLocal]
	registryPortFromLocal := labels[labelRegistryPortFromLocal]
	if registryHostnameFromLocal != "" && registryPortFromLocal != "" {
		fromLocal = fmt.Sprintf("%s:%s", registryHostnameFromLocal, registryPortFromLocal)
	}

	registryHostnameFromCluster := labels[labelRegistryHostnameFromCluster]
	registryPortFromCluster := labels[labelRegistryPortFromCluster]
	if registryHostnameFromCluster != "" && registryPortFromCluster != "" {
		fromCluster = fmt.Sprintf("%s:%s", registryHostnameFromCluster, registryPortFromCluster)
	}

	if fromLocal != "" && fromCluster != "" {
	...
	if fromLocal != "" {
		reg, err := container.NewRegistry(fromLocal)
		if err != nil {
			logger.Get(ctx).Warnf("Local registry read from node failed to parse (%s): %v", fromLocal, err)
			return container.Registry{}
		}
		return reg
	}
...
func (r *registryAsync) Registry(ctx context.Context) container.Registry {
	r.once.Do(func() {
		...
		reg := r.inferRegistryFromTiltNodeLabels(ctx)
		if !reg.Empty() {
			r.registry = reg
		}
	})
